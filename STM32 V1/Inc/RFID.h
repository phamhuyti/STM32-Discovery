...
typedef struct {
	...
	struct {
		...
		enum TagBitRates ds;		// Send D
		enum TagBitRates dr;		// Receive D
	} ta1;
...
enum StatusCode PCD_TransceiveData(byte *sendData, byte sendLen, byte *backData, byte *backLen, byte *validBits, byte rxAlig, bool checkCRC);
