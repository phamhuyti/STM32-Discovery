...
uint8_t MFRC522_Check(uint64_t *ID)
{
	uint8_t status, id[5];
	status = MFRC522_Request(PICC_REQIDL, 0); // Find cards, return card type
	if (status == MI_OK)
	{
...
uint8_t MFRC522_Request(uint8_t reqMode, uint8_t *TagType)
{
	...
	status = FindCards(&reqMode);
	return status;
}

uint8_t FindCards(uint8_t *sendData)
{
	uint8_t a;
	...
	MFRC522_WriteRegister(MFRC522_REG_FIFO_DATA, sendData[0]);
	// Execute the command
	...
	if (i != 0)
	{
		a = MFRC522_ReadRegister(MFRC522_REG_ERROR) & 0x1B;
		if (!(a))
		{
			status = MI_OK;
			if (n & irqEn & 0x01)
